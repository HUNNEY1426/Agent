...
class OpenRouterProvider extends BaseProvider {
  constructor(config = {}) {
    ...
    this.model = config.model || "liquid/lfm-2.5-2.6b:free";
  }
  ...
  async generateResponse(messages = [], options = {}) {
    ...
    const targetModel = options.model || this.model || "liquid/lfm-2.5-2.6b:free";
    ...
    try {
      ...
      // Automatic fallback if requested model is deprecated or 404
      if (
        (error.response?.status === 404 ||
          error.message?.includes("No endpoints found") ||
          error.response?.data?.error?.code === 404) &&
        targetModel !== "liquid/lfm-2.5-2.6b:free" &&
        !options._isRetry
      ) {
